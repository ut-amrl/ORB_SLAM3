...
int main(int argc, char **argv) {
  ...
  for (seq = 0; seq < num_seq; seq++) {
    ...
    for (int ni = 0; ni < nImages[seq]; ni++, proccIm++) {
      ...
      if (imLeft.empty()) {
        cerr << endl
             << "Failed to load image at: " << string(vstrImageLeft[seq][ni]) << endl;
        return 1;
      }

      if (imRight.empty()) {
        cerr << endl
             << "Failed to load image at: " << string(vstrImageRight[seq][ni]) << endl;
        return 1;
      }
      ...
      std::chrono::steady_clock::time_point t_End_Rect =
          std::chrono::steady_clock::now();
      t_rect = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
                   t_End_Rect - t_Start_Rect)
                   .count();
      SLAM.InsertRectTime(t_rect);
      t_rect = std::chrono::duration_cast<std::chrono::duration<double>>(t_End_Rect -
                                                                         t_Start_Rect)
                   .count();
#endif
