...
int main(int argc, char **argv) {
  if (argc < 3 || argc > 4) {
    cerr << endl
         << "Usage: ./stereo_inertial_realsense_t265 path_to_vocabulary "
  ...
  ORB_SLAM3::System SLAM(
      argv[1], argv[2], ORB_SLAM3::System::IMU_STEREO, true, 0, file_name);
  ...
  // Enable both image streams (for some reason realsense does not allow to
  // enable just one)
  ...
  auto imu_callback = [&](const rs2::frame &frame) {
    ...
    if (rs2::frameset fs = frame.as<rs2::frameset>()) {
      ...
      imCV = cv::Mat(cv::Size(width_img, height_img),
                     ...
                     cv::Mat::AUTO_STEP);
      imCV_right = cv::Mat(cv::Size(width_img, height_img),
                           CV_8U,
      ...
      while (v_gyro_timestamp.size() > v_accel_timestamp_sync.size()) {
        ...
        rs2_vector interp_data = interpolateMeasure(target_time,
                                                    current_accel_data,
  ...
  while (!SLAM.isShutDown()) {
...

    {
      ...
      if (count_im_buffer > 1) cout << count_im_buffer - 1 << " dropped frames\n";
      count_im_buffer = 0;
