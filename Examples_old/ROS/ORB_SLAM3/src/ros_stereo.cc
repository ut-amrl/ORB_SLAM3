...
#include <amrl_msgs/OrbSLAMSaveFilesAndStartNewTrajectorySrv.h>
...
class TrajectoryServiceListener {
public:
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr &msgLeft, const sensor_msgs::ImageConstPtr &msgRight) {
    ...
    ORB_SLAM3::Timestamp stamp = std::make_pair(cv_ptrLeft->header.stamp.sec, cv_ptrLeft->header.stamp.nsec);
    if (do_rectify) {
        cv::Mat imLeft, imRight;
        cv::remap(cv_ptrLeft->image, imLeft, M1l, M2l, cv::INTER_LINEAR);
        cv::remap(cv_ptrRight->image, imRight, M1r, M2r, cv::INTER_LINEAR);
        mpSLAM->TrackStereo(imLeft, imRight, stamp);
    } else {
        mpSLAM->TrackStereo(cv_ptrLeft->image, cv_ptrRight->image, stamp);
    }
