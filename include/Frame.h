...
class Frame {
 public:
  ...
  // Search a match for each keypoint in the left image to a keypoint in the right
  // image. If there is a match, depth is computed and the right coordinate associated
  ...
  inline Sophus::SE3<float> GetPose() const {
    // TODO: can the Frame pose be accsessed from several threads? should this be
    // protected somehow?
    return mTcw;
  }
 ...
 private:
  ...
  // translation vector: Tcw_.translation() Twc_ not necessary as Sophus has a method for
  // easily computing the inverse pose: Tcw_.inverse()
 ...
 public:
  ...
  // Vector of keypoints (original for visualization) and undistorted (actually used by
  // the system). In the stereo case, mvKeysUn is redundant as images must be rectified.
 ...
 public:
  ...
  void PrintPointDistribution() {
    ...
    cout << "Point distribution in Frame: left-> " << left << " --- right-> " << right
         << endl;
  }
