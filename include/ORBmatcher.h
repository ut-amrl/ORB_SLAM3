...
class ORBmatcher {
 public:
  ...
  // Search matches between Frame keypoints and projected MapPoints. Returns number of
  // matches Used to track the local map (Tracking)
  ...
  // Brute force constrained to ORB that belong to the same vocabulary node (at a
  ...
  // [s12*R12|t12] In the stereo and RGB-D case, s12=1 int SearchBySim3(KeyFrame* pKF1,
  // KeyFrame* pKF2, std::vector<MapPoint *> &vpMatches12, const float &s12, const
  ...
  // Project MapPoints into KeyFrame using a given Sim3 and search for duplicated
  // MapPoints.
  int Fuse(KeyFrame *pKF,
           Sophus::Sim3f &Scw,
           const std::vector<MapPoint *> &vpPoints,
