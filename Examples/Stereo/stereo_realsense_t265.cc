...
int main(int argc, char **argv) {
  if (argc < 3 || argc > 4) {
    cerr << endl
         << "Usage: ./stereo_realsense_t265 path_to_vocabulary "
  ...
  ORB_SLAM3::System SLAM(
      argv[1], argv[2], ORB_SLAM3::System::STEREO, true, 0, file_name);
  float imageScale = SLAM.GetImageScale();

  cv::Mat imLeft, imRight;
  vector<ORB_SLAM3::IMU::Point> vImuMeas;

  rs2::stream_profile fisheye_stream_left =
      pipe_profile.get_stream(RS2_STREAM_FISHEYE, 1);
  rs2_intrinsics intrinsics_left =
      fisheye_stream_left.as<rs2::video_stream_profile>().get_intrinsics();
  int width_left = intrinsics_left.width;
  int height_left = intrinsics_left.height;

  rs2::stream_profile fisheye_stream_right =
      pipe_profile.get_stream(RS2_STREAM_FISHEYE, 2);
  rs2_intrinsics intrinsics_right =
      fisheye_stream_right.as<rs2::video_stream_profile>().get_intrinsics();
  ...
  while (b_continue_session) {
    ...
    if (rs2::video_frame image_frame = frame_set.first_or_default(RS2_STREAM_FISHEYE)) {
      ...
      imLeft = cv::Mat(cv::Size(width_left, height_left),
                       CV_8UC1,
                       (void *)(frame_left.get_data()),
                       cv::Mat::AUTO_STEP);
      imRight = cv::Mat(cv::Size(width_right, height_right),
                        CV_8UC1,
