  ...
  for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) {
    ...
    if (pMP->mbTrackInView) {
      ...
      if (!vIndices.empty()) {
        ...
        // Apply ratio to second match (only if best and second are in the same scale
        // level)
        if (bestDist <= TH_HIGH) {
...
int ORBmatcher::SearchByBoW(KeyFrame *pKF,
  ...
  // We perform the matching over ORB that belong to the same vocabulary node (at a
  // certain level)
...
int ORBmatcher::SearchForInitialization(Frame &F1,
  ...
  for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++) {
    ...
    for (vector<size_t>::iterator vit = vIndices2.begin(); vit != vIndices2.end();
         vit++) {
      size_t i2 = *vit;
    ...
    if (pMP) {
      if (!LastFrame.mvbOutlier[i]) {
        ...
        if (bForward)
          ...
          vIndices2 =
              CurrentFrame.GetFeaturesInArea(uv(0), uv(1), radius, 0, nLastOctave);
        else
        ...
        if (CurrentFrame.Nleft != -1) {
          ...
          for (vector<size_t>::const_iterator vit = vIndices2.begin(),
            ...
            if (CurrentFrame.mvpMapPoints[i2 + CurrentFrame.Nleft])
              if (CurrentFrame.mvpMapPoints[i2 + CurrentFrame.Nleft]->Observations() >
                  0)
                continue;
