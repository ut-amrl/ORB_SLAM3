...
void LocalMapping::CreateNewMapPoints() {
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++) {
    ...
    for (int ikp = 0; ikp < nmatches; ikp++) {
      ...
      const bool bRight1 =
          (mpCurrentKeyFrame->NLeft == -1 || idx1 < mpCurrentKeyFrame->NLeft) ? false
                                                                              : true;
  ...
  if (bFIBA) {
    if (priorA != 0.f)
      ...
      Optimizer::FullInertialBA(
          mpAtlas->GetCurrentMap(), 100, false, mpCurrentKeyFrame->mnId, NULL, false);
  }
...
void LocalMapping::ScaleRefinement() {
  ...
  // Minimum time (seconds) between first and last keyframe to compute a solution. Make
  // the difference between monocular and stereo unique_lock<mutex>
  // lock0(mMutexImuInit);
  if (mbResetRequested) return;
