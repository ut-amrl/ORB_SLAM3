...
void Tracking::LocalMapStats2File() {
  ...
  f << "#Stereo rect[ms], MP culling[ms], MP creation[ms], LBA[ms], KF culling[ms], "
       "Total[ms]"
    << endl;
  for (int i = 0; i < mpLocalMapper->vdLMTotal_ms.size(); ++i) {
    f << mpLocalMapper->vdKFInsert_ms[i] << "," << mpLocalMapper->vdMPCulling_ms[i]
      << "," << mpLocalMapper->vdMPCreation_ms[i] << ","
      << mpLocalMapper->vdLBASync_ms[i] << "," << mpLocalMapper->vdKFCullingSync_ms[i]
...
void Tracking::TrackStats2File() {
  ...
  for (int i = 0; i < vdTrackTotal_ms.size(); ++i) {
    ...
    f << stereo_rect << "," << resize_image << "," << vdORBExtract_ms[i] << ","
      ...
      << vdLMTrack_ms[i] << "," << vdNewKF_ms[i] << "," << vdTrackTotal_ms[i] << endl;
  }
...
bool Tracking::ParseCamParamFile(cv::FileStorage& fSettings) {
  ...
  if (sCameraName == "PinHole") {
    ...
    if (mSensor == System::STEREO || mSensor == System::IMU_STEREO ||
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*Camera2.fx parameter doesn't exist or is not a real number*"
                  << std::endl;
        b_miss_params = true;
      }
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*Camera2.fy parameter doesn't exist or is not a real number*"
                  << std::endl;
        b_miss_params = true;
      }
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*Camera2.cx parameter doesn't exist or is not a real number*"
                  << std::endl;
        b_miss_params = true;
      }
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*Camera2.cy parameter doesn't exist or is not a real number*"
                  << std::endl;
        b_miss_params = true;
      }
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*Camera2.k1 parameter doesn't exist or is not a real number*"
                  << std::endl;
        b_miss_params = true;
      }
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*Camera2.k2 parameter doesn't exist or is not a real number*"
                  << std::endl;
        b_miss_params = true;
      }
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*Camera2.k3 parameter doesn't exist or is not a real number*"
                  << std::endl;
        b_miss_params = true;
      }
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*Camera2.k4 parameter doesn't exist or is not a real number*"
                  << std::endl;
        b_miss_params = true;
      }
      ...
      if (!b_miss_params) {
        ...
        std::cout << "- Camera1 Lapping: " << leftLappingBegin << ", " << leftLappingEnd
                  << std::endl;
  ...
  if (mSensor == System::RGBD || mSensor == System::IMU_RGBD) {
    ...
    if (!node.empty() && node.isReal()) {
      ...
      std::cerr << "*DepthMapFactor parameter doesn't exist or is not a real number*"
                << std::endl;
      b_miss_params = true;
    }
  }
...
bool Tracking::ParseORBParamFile(cv::FileStorage& fSettings) {
  ...
  if (!node.empty() && node.isInt()) {
    ...
    std::cerr << "*ORBextractor.nFeatures parameter doesn't exist or is not an integer*"
              << std::endl;
    b_miss_params = true;
  }
  ...
  if (!node.empty() && node.isReal()) {
    ...
    std::cerr
        << "*ORBextractor.scaleFactor parameter doesn't exist or is not a real number*"
        << std::endl;
    b_miss_params = true;
  }
  ...
  if (!node.empty() && node.isInt()) {
    ...
    std::cerr << "*ORBextractor.nLevels parameter doesn't exist or is not an integer*"
              << std::endl;
    b_miss_params = true;
  }
  ...
  if (!node.empty() && node.isInt()) {
    ...
    std::cerr << "*ORBextractor.iniThFAST parameter doesn't exist or is not an integer*"
              << std::endl;
    b_miss_params = true;
  }
  ...
  if (!node.empty() && node.isInt()) {
    ...
    std::cerr << "*ORBextractor.minThFAST parameter doesn't exist or is not an integer*"
              << std::endl;
    b_miss_params = true;
  }
...
bool Tracking::ParseIMUParamFile(cv::FileStorage& fSettings) {
  ...
  if (!node.empty() && node.isReal()) {
    ...
    std::cerr << "*IMU.NoiseGyro parameter doesn't exist or is not a real number*"
              << std::endl;
    b_miss_params = true;
  }
  ...
  if (!node.empty() && node.isReal()) {
    ...
    std::cerr << "*IMU.NoiseAcc parameter doesn't exist or is not a real number*"
              << std::endl;
    b_miss_params = true;
  }
  ...
  if (!node.empty() && node.isReal()) {
    ...
    std::cerr << "*IMU.GyroWalk parameter doesn't exist or is not a real number*"
              << std::endl;
    b_miss_params = true;
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    if ((i == 0) && (i < (n - 1))) {
      ...
      acc = (mvImuFromLastFrame[i].a + mvImuFromLastFrame[i + 1].a -
            ...
            0.5f;
      angVel =
          (mvImuFromLastFrame[i].w + mvImuFromLastFrame[i + 1].w -
...
bool Tracking::PredictStateIMU() {
  ...
  if (mbMapUpdated && mpLastKeyFrame) {
    ...
    Eigen::Matrix3f Rwb2 =
        IMU::NormalizeRotation(Rwb1 * mpImuPreintegratedFromLastKF->GetDeltaRotation(
    ...
    Eigen::Matrix3f Rwb2 = IMU::NormalizeRotation(
        Rwb1 *
        mCurrentFrame.mpImuPreintegratedFrame->GetDeltaRotation(mLastFrame.mImuBias));
    Eigen::Vector3f twb2 =
...
void Tracking::Track() {
  ...
  if (mState == NOT_INITIALIZED) {
    ...
    // Initial camera pose estimation using motion model or relocalization (if tracking
    // is lost)
    if (!mbOnlyTracking) {
      // State OK
    ...
    std::chrono::steady_clock::time_point time_StartLMTrack =
        std::chrono::steady_clock::now();
#endif
    // If we have an initial estimation of the camera pose and matching. Track the local
    // map.
    if (!mbOnlyTracking) {
      if (bOK) {
        bOK = TrackLocalMap();
    ...
    // Save frame if recent relocalization, since they are used for IMU reset (as we are
    // making copy, it shluld be once mCurrFrame is completely modified)
    if ((mCurrentFrame.mnId < (mnLastRelocFrameId + mnFramesToResetIMU)) &&
    ...
    if (bOK || mState == RECENTLY_LOST) {
      ...
      // Function) pass to the new keyframe, so that bundle adjustment will finally
      // decide if they are outliers or not. We don't want next frame to estimate its
      // position with those points so we discard them in the frame. Only has effect if
      // lastframe is tracked
      for (int i = 0; i < mCurrentFrame.N; i++) {
        if (mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
          mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
      }
...
void Tracking::CreateMapInAtlas() {
  ...
  mbVelocity = false;
  // mnLastRelocFrameId = mnLastInitFrameId; // The last relocation KF_id is the current
  // id, because it is the new starting point for new map
  Verbose::PrintMess("First frame id in map: " + to_string(mnLastInitFrameId + 1),
                     Verbose::VERBOSITY_NORMAL);
...
bool Tracking::NeedNewKeyFrame() {
  ...
  // Check how many "close" points are being tracked and how many could be potentially
  // created.
  ...
  if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR) {
    ...
    // Verbose::PrintMess("[NEEDNEWKF]-> closed points: " + to_string(nTrackedClose) +
    // "; non tracked closed points: " + to_string(nNonTrackedClose),
    // Verbose::VERBOSITY_NORMAL);// Verbose::VERBOSITY_DEBUG);
  }
  ...
  {
      //Pseudo-monocular, there are not enough close points to be confident about the
  stereo observations. thRefRatio = 0.9f;
  }*/
  ...
  // Condition 2: Few tracked points compared to reference keyframe. Lots of visual
  // odometry compared to map matches.
  const bool c2 =
  ...
  if ((((mnMatchesInliers < 75) && (mnMatchesInliers > 15)) ||
       mState == RECENTLY_LOST) &&
      (mSensor ==
       System::IMU_MONOCULAR))  // MODIFICATION_2, originally ((((mnMatchesInliers<75)
...
void Tracking::UpdateLocalKeyFrames() {
  ...
  // All keyframes that observe a map point are included in the local map. Also check
  // which keyframe shares most points
