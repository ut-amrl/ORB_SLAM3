...
void Frame::AssignFeaturesToGrid() {
  ...
  for (int i = 0; i < N; i++) {
    const cv::KeyPoint &kp =
        (Nleft == -1) ? mvKeysUn[i] : (i < Nleft) ? mvKeys[i] : mvKeysRight[i - Nleft];
...
void Frame::UndistortKeyPoints() {
  ...
  mat = mat.reshape(2);
  cv::undistortPoints(
      mat, mat, static_cast<Pinhole *>(mpCamera)->toK(), mDistCoef, cv::Mat(), mK);
  mat = mat.reshape(1);
...
void Frame::ComputeStereoFishEyeMatches() {
  ...
  for (vector<vector<cv::DMatch>>::iterator it = matches.begin(); it != matches.end();
       ++it) {
