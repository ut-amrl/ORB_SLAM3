...
System::System(const string& strVocFile,
  ...
  cout << endl
       << "ORB-SLAM3 Copyright (C) 2017-2020 Carlos Campos, Richard Elvira, Juan J. "
          "Gómez, José M.M. Montiel and Juan D. Tardós, University of Zaragoza."
       << endl
       << "ORB-SLAM2 Copyright (C) 2014-2016 Raúl Mur-Artal, José M.M. Montiel and "
...
Sophus::SE3f System::TrackStereo(const cv::Mat& imLeft,
  ...
  if (mSensor != STEREO && mSensor != IMU_STEREO) {
    cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor "
            "Stereo-Inertial."
         << endl;
    exit(-1);
  }
...
void System::SaveTrajectoryTUM(const string& filename) {
  ...
  // Frame pose is stored relative to its reference keyframe (which is optimized by BA
  // and pose graph). We need to get first the keyframe pose and then concatenate the
  // relative transformation. Frames not localized (tracking failure) are not saved.
  ...
  for (list<Sophus::SE3f>::iterator lit = mpTracker->mlRelativeFramePoses.begin(),
    ...
    f << setprecision(6) << *lT << " " << setprecision(9) << twc(0) << " " << twc(1)
      << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w()
      << endl;
  }
  f.close();
...
void System::SaveTrajectoryEuRoC(const string& filename) {
  ...
  int numMaxKFs = 0;
  Map* pBiggerMap;
  std::cout << "There are " << std::to_string(vpMaps.size()) << " maps in the atlas"
            << std::endl;
  for (Map* pMap : vpMaps) {
  ...
  // cout << "size mlRelativeFramePoses: " << mpTracker->mlRelativeFramePoses.size() <<
  // endl; cout << "size mpTracker->mlFrameTimes: " << mpTracker->mlFrameTimes.size() <<
  // endl; cout << "size mpTracker->mlbLost: " << mpTracker->mlbLost.size() << endl;
...
endl;
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        // If the reference keyframe was culled, traverse the spanning tree to get a
suitable keyframe. if (!pKF) continue;
        ...
        Trw = Trw * pKF->GetPose()*Twb; // Tcp*Tpw*Twb0=Tcb0 where b0 is the new world
reference
...
twb.at<float>(0) << " " << twb.at<float>(1) << " " << twb.at<float>(2) << " " << q[0] <<
" " << q[1] << " " << q[2] << " " << q[3] << endl;
...
void System::SaveKeyFrameTrajectoryEuRoC(const string& filename) {
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD) {
      ...
      Eigen::Quaternionf q = Twb.unit_quaternion();
      Eigen::Vector3f twb = Twb.translation();
      f << setprecision(6) << 1e9 * pKF->mTimeStamp << " " << setprecision(9) << twb(0)
        << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " "
        << q.z() << " " << q.w() << endl;
      ...
      Eigen::Quaternionf q = Twc.unit_quaternion();
      Eigen::Vector3f t = Twc.translation();
      f << setprecision(6) << 1e9 * pKF->mTimeStamp << " " << setprecision(9) << t(0)
        << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y() << " " << q.z()
        << " " << q.w() << endl;
    }
  }
...
void System::SaveKeyFrameTrajectoryEuRoC(const string& filename, Map* pMap) {
  cout << endl
       << "Saving keyframe trajectory of map " << pMap->GetId() << " to " << filename
       << " ..." << endl;
...
void System::SaveTrajectoryKITTI(const string& filename) {
  ...
  for (list<Sophus::SE3f>::iterator lit = mpTracker->mlRelativeFramePoses.begin(),
    ...
    f << setprecision(9) << Rwc(0, 0) << " " << Rwc(0, 1) << " " << Rwc(0, 2) << " "
      << twc(0) << " " << Rwc(1, 0) << " " << Rwc(1, 1) << " " << Rwc(1, 2) << " "
      << twc(1) << " " << Rwc(2, 0) << " " << Rwc(2, 1) << " " << Rwc(2, 2) << " "
      << twc(2) << endl;
  }
  f.close();
}
...
void System::SaveDebugData(const int& initIdx) {
  ...
  SaveTrajectoryEuRoC("init_FrameTrajectoy_" + to_string(mpLocalMapper->mInitSect) +
                      "_" + to_string(initIdx) + ".txt");
...
bool System::LoadAtlas(int type) {
  ...
  if (isRead) {
    ...
    if (strInputVocabularyChecksum.compare(strVocChecksum) != 0) {
      cout << "The vocabulary load isn't the same which the load session was created "
           << endl;
