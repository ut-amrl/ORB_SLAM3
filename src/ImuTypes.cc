...
IntegratedRotation::IntegratedRotation(const Eigen::Vector3f &angVel,
  ...
  if (d < eps) {
    ...
    deltaR =
        Eigen::Matrix3f::Identity() + W * sin(d) / d + W * W * (1.0f - cos(d)) / d2;
...
void Preintegrated::IntegrateNewMeasurement(const Eigen::Vector3f &acceleration,
  ...
  // Position is updated firstly, as it depends on previously computed velocity and
  // rotation. Velocity is updated secondly, as it depends on previously computed
  // rotation. Rotation is the last to be updated.
  ...
  // Compute velocity and position parts of matrices A and B (rely on non-updated delta
  // rotation)
