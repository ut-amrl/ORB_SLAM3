...
bool LoopClosing::NewDetectCommonRegions() {
  ...
  {
    // cout << "LoopClousure: Stereo KF inserted without check: " << mpCurrentKF->mnId
    // << endl;
    mpKeyFrameDB->add(mpCurrentKF);
    mpCurrentKF->SetErase();
    return false;
  ...
  if (mnLoopNumCoincidences > 0) {
    ...
    bool bCommonRegion = DetectAndReffineSim3FromLastKF(
        mpCurrentKF, mpLoopMatchedKF, gScw, numProjMatches, mvpLoopMPs, vpMatchedMPs);
    if (bCommonRegion) {
  ...
  if (mnMergeNumCoincidences > 0) {
    ...
    vector<MapPoint*> vpMatchedMPs;
    bool bCommonRegion = DetectAndReffineSim3FromLastKF(
        mpCurrentKF, mpMergeMatchedKF, gScw, numProjMatches, mvpMergeMPs, vpMatchedMPs);
    if (bCommonRegion) {
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF,
                                                 KeyFrame* pMatchedKF,
                                                 g2o::Sim3& gScw,
                                                 int& nNumProjMatches,
  ...
  if (nNumProjMatches >= nProjMatches) {
    // Verbose::PrintMess("Sim3 reffine: There are " + to_string(nNumProjMatches) + "
    // initial matches ", Verbose::VERBOSITY_DEBUG);
...
bool LoopClosing::DetectCommonRegionsFromBoW(std::vector<KeyFrame*>& vpBowCand,
  ...
  int index = 0;
  // Verbose::PrintMess("BoW candidates: There are " + to_string(vpBowCand.size()) + "
  // possible candidates ", Verbose::VERBOSITY_DEBUG);
  for (KeyFrame* pKFi : vpBowCand) {
    ...
    if (numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
    {
      ...
      while (!bConverge && !bNoMore) {
        ...
        // geometrical inliers among " + to_string(nBoWInliers) + " BoW matches",
      ...
      if (bConverge) {
        ...
        // Verbose::PrintMess("BoW guess: Convergende with " + to_string(nInliers) + "
        // geometrical inliers among " + to_string(nBoWInliers) + " BoW matches",
        // Verbose::VERBOSITY_DEBUG);
        ...
        // std::cout << "There are " << vpKeyFrames.size() <<" KFs which view all the
        // mappoints" << std::endl;
        ...
        if (numProjMatches >= nProjMatches) {
          ...
          int numOptMatches = Optimizer::OptimizeSim3(
              mpCurrentKF, pKFi, vpMatchedMP, gScm, 10, mbFixScale, mHessian7x7, true);
      ...
      /*else
      {
          Verbose::PrintMess("BoW candidate: it don't match with the current one",
      Verbose::VERBOSITY_DEBUG);
      }*/
    }
    index++;
...
bool LoopClosing::DetectCommonRegionsFromLastKF(KeyFrame* pCurrentKF,
                                                KeyFrame* pMatchedKF,
                                                g2o::Sim3& gScw,
                                                int& nNumProjMatches,
                                                std::vector<MapPoint*>& vpMPs,
...
void LoopClosing::CorrectLoop() {
  ...
  // Retrive keyframes connected to the current keyframe and compute corrected Sim3 pose
  // by propagation
  ...
  g2o::Sim3 g2oScw(
      Tcw.unit_quaternion().cast<double>(), Tcw.translation().cast<double>(), 1.0);
...

  {
    ...
    // Correct all MapPoints obsrved by current keyframe and neighbors, so that they
    // align with the other side of the loop
  ...
  // Launch a new thread to perform Global Bundle Adjustment (Only if few keyframes, if
  // not it would take too much time)
...
void LoopClosing::MergeLocal() {
  ...
  // Relationship to rebuild the essential graph, it is used two times, first in the
  // local window and later in the rest of the map
  KeyFrame* pNewChild;
  KeyFrame* pNewParent;
  ...
  // Flag that is true only when we stopped a running BA, in this case we need relaunch
  ...
  // Merge map will become in the new active map with the local window of KFs and MPs
  // from the current map. Later, the elements of the current map will be transform to
  // the new active map reference, in order to keep real time tracking
  ...
  for (KeyFrame* pKFi : spLocalWindowKFs) {
    ...
    if (pCurrentMap->isImuInitialized()) {
      Eigen::Quaternionf Rcor =
          (g2oCorrectedSiw.rotation().inverse() * vNonCorrectedSim3[pKFi].rotation())
              .cast<float>();
  ...
  {
      std::cout << "[Merge]: " << std::to_string(numPointsWithCorrection) << " points
  removed from Ma due to its reference KF is not in welding area" << std::endl;
      std::cout << "[Merge]: Ma has " << std::to_string(spLocalWindowMPs.size()) << "
  points" << std::endl;
  }*/

  {
    unique_lock<mutex> currentLock(
        pCurrentMap
            ->mMutexMapUpdate);  // We update the current map with the Merge information
    unique_lock<mutex> mergeLock(
        pMergeMap->mMutexMapUpdate);  // We remove the Kfs and MPs in the merged area
                                      // from the old map
  ...
  pNewChild =
      mpCurrentKF->GetParent();  // Old parent, it will be the new child of this KF
  pNewParent =
      mpCurrentKF;  // Old child, now it will be the parent of its own parent(we need
                    // eliminate this KF from children list in its old parent)
  mpCurrentKF->ChangeParent(mpMergeMatchedKF);
  while (pNewChild) {
    pNewChild->EraseChild(pNewParent);  // We remove the relation between the old parent
                                        // and the new for avoid loop
  ...
  if (vpCurrentMapKFs.size() == 0) {
    ...
    if (mpTracker->mSensor == System::MONOCULAR) {
      unique_lock<mutex> currentLock(
          pCurrentMap->mMutexMapUpdate);  // We update the current map with the Merge
                                          // information
    ...
    // Optimize graph (and update the loop position for each element form the begining
    // to the end)
    if (mpTracker->mSensor != System::MONOCULAR) {
  ...
  if (bRelaunchBA &&
    ...
    mbStopGBA = false;
    mpThreadGBA = new thread(
        &LoopClosing::RunGlobalBundleAdjustment, this, pMergeMap, mpCurrentKF->mnId);
  }
...
void LoopClosing::MergeLocal2() {
  ...
  int numTemporalKFs = 11;  // TODO (set by parameter): Temporal KFs in the local window
                            // if the map is inertial.
  ...
  // cout << "MergeMap init ID: " << pMergeMap->GetInitKFid() << "       CurrMap init
  // ID: " << pCurrentMap->GetInitKFid() << endl;
  ...
  pNewChild =
      mpMergeMatchedKF->GetParent();  // Old parent, it will be the new child of this KF
  pNewParent =
      mpMergeMatchedKF;  // Old child, now it will be the parent of its own parent(we
                         // need eliminate this KF from children list in its old parent)
...
void LoopClosing::SearchAndFuse(const vector<KeyFrame*>& vConectedKFs,
  ...
  for (auto mit = vConectedKFs.begin(), mend = vConectedKFs.end(); mit != mend; mit++) {
    ...
    /*cout << "FUSE-POSE: KF " << pKF->mnId << " ->" << num_replaces << " MPs fused" <<
    endl; total_replaces += num_replaces;*/
  }
...
void LoopClosing::ResetIfRequested() {
  ...
  if (mbResetRequested) {
    ...
    mLastLoopKFid = mpAtlas->GetLastInitKFid();  // TODO old variable, it is not use in
                                                 // the new algorithm
    mbResetActiveMapRequested = false;
  }
}
  ...
  // We need to propagate the correction through the spanning tree
  {
    ...
    if (!mbStopGBA) {
      ...
      while (!lpKFtoCheck.empty()) {
        ...
        // childs" << endl; cout << " KF mnBAGlobalForKF: " << pKF->mnBAGlobalForKF <<
        // endl;
        ...
        for (set<KeyFrame*>::const_iterator sit = sChilds.begin(); sit != sChilds.end();
          ...
          if (pChild->mnBAGlobalForKF != nLoopKF) {
            ...
            // LoopKF: " << nLoopKF << endl; cout << " child id: " << pChild->mnId <<
            // endl;
        ...
        cout << "GBA: KF " << pKF->mnId << " had been moved " << dist << " meters" <<
        endl; double desvX = 0; double desvY = 0; double desvZ = 0;
        if(pKF->mbHasHessian)
        {
        ...
        pKF->GetConnectedKeyFrames().size() << " connected KFs" << endl; cout << "--It
        has " << pKF->GetCovisiblesByWeight(80).size() << " connected KF with 80 common
        matches or more" << endl; cout << "--It has " <<
        pKF->GetCovisiblesByWeight(50).size() << " connected KF with 50 common matches
        or more" << endl; cout << "--It has " << pKF->GetCovisiblesByWeight(20).size()
        ...
        to_string(pKF->mnId) +"_D" + to_string(dist) +".png"; cv::imwrite(namefile,
        imLeft);
        }*/
