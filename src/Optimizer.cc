...
void Optimizer::OptimizeEssentialGraph(
  ...
  // Correct points. Transform to "non-optimized" reference keyframe pose and transform
  // back with optimized pose
...
void Optimizer::OptimizeEssentialGraph(KeyFrame* pCurKF,
  ...
  Verbose::PrintMess("Opt_Essential: There are " + to_string(vpFixedKFs.size()) +
...
void Optimizer::LocalInertialBA(KeyFrame* pKF,
  ...
  for (list<KeyFrame*>::iterator it = lpOptVisKFs.begin(), itEnd = lpOptVisKFs.end();
       it != itEnd;
       it++) {
    KeyFrame* pKFi = *it;
    VertexPose* VP = new VertexPose(pKFi);
  ...
  for (list<KeyFrame*>::iterator lit = lFixedKeyFrames.begin(),
    ...
    if (pKFi->bImu)  // This should be done only for keyframe just before temporal
                     // window
    {
      VertexVelocity* VV = new VertexVelocity(pKFi);
      VV->setId(maxKFid + 3 * (pKFi->mnId) + 1);
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
      ...
      if (i == N - 1 || bRecInit) {
        // All inertial residuals are included without robust cost function, but not
        // that one linking the last optimizable keyframe inside of the local window and
        // the first fixed keyframe out. The information matrix for this measurement is
        // also downweighted. This is done to avoid accumulating error due to fixing
        // variables.
        g2o::RobustKernelHuber* rki = new g2o::RobustKernelHuber;
...
void Optimizer::InertialOptimization(Map* pMap,
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    VertexVelocity* VV =
        static_cast<VertexVelocity*>(optimizer.vertex(maxKFid + (pKFi->mnId) + 1));
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,
                                      ...
                                      bool* pbStopFlag) {
  printf("LocalBundleAdjustment 2\n");
  ...
  Verbose::PrintMess("[BA]: Second optimization, there are " + to_string(badMonoMP) +
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF,
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (vector<MapPoint*>::iterator vit = vpMPs.begin(), vend = vpMPs.end();
         vit != vend;
         vit++) {
      // Using mnBALocalForKF we avoid redundance here, one MP can not be added several
      // times to lLocalMapPoints
      MapPoint* pMP = *vit;
      if (pMP)
        if (!pMP->isBad())
  ...
  // Keyframes
  int i = 0;
  for (vector<pair<MapPoint*, int>>::iterator lit = pairs.begin(), lend = pairs.end();
       lit != lend;
       lit++, i++) {
...
void Optimizer::OptimizeEssentialGraph4DoF(
  ...
  // Edge used in posegraph has still 6Dof, even if updates of camera poses are just in
  // 4DoF
