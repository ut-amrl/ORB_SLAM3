...
void KeyFrame::SetBadFlag() {
...

  {
    ...
    // Assign at each iteration one children with a parent (the pair with highest
    // covisibility weight) Include that children as new parent candidate for the rest
    ...
    // If a children has no covisibility links with any parent candidate, assign to the
    // original parent of this KF
...
void KeyFrame::PostLoad(map<long unsigned int, KeyFrame *> &mpKFid,
  ...
  // KeyFrame childrens
  mspChildrens.clear();
  for (vector<long unsigned int>::const_iterator it = mvBackupChildrensId.begin(),
  ...
  // Loop edge KeyFrame
  mspLoopEdges.clear();
  for (vector<long unsigned int>::const_iterator it = mvBackupLoopEdgesId.begin(),
  ...
  // Merge edge KeyFrame
  mspMergeEdges.clear();
  for (vector<long unsigned int>::const_iterator it = mvBackupMergeEdgesId.begin(),
