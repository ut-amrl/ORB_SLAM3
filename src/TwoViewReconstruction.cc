...
Eigen::Matrix3f TwoViewReconstruction::ComputeH21(const vector<cv::Point2f> &vP1,
...
Eigen::Matrix3f TwoViewReconstruction::ComputeF21(const vector<cv::Point2f> &vP1,
                                                  const vector<cv::Point2f> &vP2) {
  const int N = vP1.size();
...
bool TwoViewReconstruction::ReconstructH(vector<bool> &vbMatchesInliers,
  ...
  // (which could fail for points seen with low parallax) We reconstruct all hypotheses
...
int TwoViewReconstruction::CheckRT(const Eigen::Matrix3f &R,
  ...
  for (size_t i = 0, iend = vMatches12.size(); i < iend; i++) {
    ...
    // Check depth in front of first camera (only if enough parallax, as "infinite"
    // points can easily go to negative depth)
    if (p3dC1(2) <= 0 && cosParallax < 0.99998) continue;

    // Check depth in front of second camera (only if enough parallax, as "infinite"
    // points can easily go to negative depth)
